...
public class SensorApp extends Applet {
	...
	public void startup() {
		System.out.println("- - - - - - - Dthing Data Collection Function Display Ser47- - - - - - -");
		startMqtt();
		handleSensor();
	}
	...
	public String parser(byte[] data){
		...
		if(mqtt != null){
			if(mqtt.isConnected()){
				...
				int value = byte2ToInt(data[16],data[17]);
				int inte  = value/100;
				int deci  = value%100;
				String temp = "" + inte + "." + (deci < 10 ? ("0" + deci) : ("" + deci));
				
				value = byte2ToInt(data[18],data[19]);
				inte  = value/100;
				deci  = value%100;
				String humi = "" + inte + "." + (deci < 10 ? ("0" + deci) : ("" + deci));			
				System.out.println("here is the sensor's data:[temp:"+temp+",humi:"+humi+"]");
				try {
					mqtt.publish(TOPIC, ("device["+ip+"] collected data info[temp:"+temp+",humi:"+humi+"]").getBytes(), 0, false);
				} catch (MqttException e) {
					e.printStackTrace();
				}
			}else{
